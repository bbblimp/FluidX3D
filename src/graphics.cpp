...
INT WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ PSTR, _In_ INT) {
	...
	if(g_args["window"].as<bool>()) {
		width = (uint)g_args["FRAME_WIDTH"].as<int>();
		height = (uint)g_args["FRAME_HEIGHT"].as<int>();
#if defined(_WIN32)
		DWORD exStyle = 0;
	...
	if(g_args["window"].as<bool>()) {
		...
		window = CreateWindow("WindowClass", WINDOW_NAME, WS_POPUP|WS_VISIBLE, mi.rcMonitor.left, mi.rcMonitor.top, (int)width, (int)height, 0, 0, hInstance, 0); // create fullscreen window
	}
