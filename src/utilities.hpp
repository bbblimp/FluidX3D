...
inline vector<string> get_main_arguments(int argc, char* argv[]) {
    ...
    options.add_options()
        ...
        ("VIS_LATTICE", "Enable lattice/box edge rendering at startup (0/1)", cxxopts::value<int>()->default_value("0"))
        ("VIS_STREAMLINES", "Enable streamline rendering at startup (0/1)", cxxopts::value<int>()->default_value("0"))
        ("VIS_SLICE_MODE", "Streamline/field slice mode (0=no slice, 1=x, 2=y, 3=z, 4=xz, 5=xyz, 6=yz, 7=xy)", cxxopts::value<int>()->default_value("0"))
        ("VIS_SLICE_X", "Slice index in X (negative means center)", cxxopts::value<int>()->default_value("-1"))
        ("VIS_SLICE_Y", "Slice index in Y (negative means center)", cxxopts::value<int>()->default_value("-1"))
